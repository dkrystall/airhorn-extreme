...
/*
	...
	presets.
 */
public class AudioSingleton : NSObject {
    ...
    public init(componentType inComponentType: UInt32, componentsFoundCallback inComponentsFoundCallback: ((Void) -> Void)? = nil) {
        ...
        if isEffect() {
            guard let fileURL = Bundle.main.url(forResource: "airhorn_default", withExtension: "wav") else {
                fatalError("\"airhorn_default.wav\" file not found.")
            }
